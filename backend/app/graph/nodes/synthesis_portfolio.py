...
async def synthesis_portfolio_node(state: ResearchState) -> dict:
    ...
    prompt = (
        ...
        "with: (1) a brief per-stock summary for each holding, and (2) a portfolio-level "
        "section noting any sector concentration or overlap across the holdings (purely "
        "qualitative — do not compute correlations, optimal weights, or other "
        f"quantitative portfolio math). {citation_instruction(all_findings)}{failed_note} "
        "If a section says 'Unavailable', mention that gap rather than omitting it. Do "
